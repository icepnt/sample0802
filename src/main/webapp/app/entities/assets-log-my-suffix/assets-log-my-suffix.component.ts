...
export class AssetsLogMySuffixComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.assetsLogService
            ...
            .subscribe(
                (res: HttpResponse<IAssetsLogMySuffix[]>) => this.paginateAssetsLogs(res.body, res.headers),
    ...
    registerChangeInAssetsLogs() {
        this.eventSubscriber = this.eventManager.subscribe('assetsLogListModification', response => this.reset());
    }
    ...
    private paginateAssetsLogs(data: IAssetsLogMySuffix[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        for (let i = 0; i < data.length; i++) {
            this.assetsLogs.push(data[i]);
        }
    }
